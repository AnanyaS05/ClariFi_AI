...
def parse_action(line: str) -> Optional[Tuple[str, Dict[str, Any]]]:
    ...
    # 🔹 Tolerate an accidental trailing ">"
    # e.g. 'finish[answer="..."]>'
    if rest.endswith(">"):
        rest = rest[:-1].rstrip()
    ...
    # 🔹 Special-case finish[answer="..."] so commas, etc. don’t break it
    if name == "finish":
        # Expected format: answer="...anything..."
        m_ans = re.match(r'^answer\s*=\s*"(.*)"\s*$', argstr, flags=re.DOTALL)
        if not m_ans:
            return None
        answer_text = m_ans.group(1)
        return name, {"answer": answer_text}

    # If empty arg string, return empty args for other tools
    if argstr == "":
        return name, {}
...
def format_history(trajectory: List[Dict[str, str]]) -> str:
    ...
    Assumes:
      - step["thought"] is raw text (no 'Thought:' prefix)
      - step["action"] ALREADY includes the 'Action: ' prefix
    """
    lines: List[str] = []
    for step in trajectory:
        lines.append(f"Thought: {step['thought']}")
        # action string already has "Action: ..."
...
SYSTEM_PREAMBLE = textwrap.dedent("""\
    You are a helpful ReAct agent that explains financial information to everyday people.

    Available tools:
    - search[query="<text>", k=<int>]
        # searches a small encyclopedia of financial documents and returns top-k results
    - explain_term[term="<financial term>"]
        # explains financial jargon in simple, everyday language
    To finish, use: finish[answer="<final answer>"]

    IMPORTANT RULES (DO NOT REPEAT THESE RULES IN YOUR THOUGHT OR ANSWER):
    1. At each step you MUST output EXACTLY:
         - ONE line starting with "Thought:"
         - ONE line starting with "Action:"
    2. The Action line MUST contain EXACTLY ONE tool call:
         - search[query="<text>", k=<int>]
         - explain_term[term="<financial term>"]
         - finish[answer="<final answer>"]
       There must NEVER be more than one tool call per Action line.
       ❌ Incorrect: Action: search[...] finish[answer="..."]
    3. The Action line must contain ONLY the tool call. No extra words, no explanations.
    4. Use explain_term[term="..."] whenever a financial term might confuse a non-expert
       (for example: EPS, operating margin, guidance, cash flow, EBITDA, etc.).
    5. THOUGHT is your private reasoning. In Thought:
         - Reason briefly about what the user is really asking.
         - Decide whether to call search, explain_term, or finish.
         - DO NOT copy these rules, and DO NOT talk about “answer length” or “style”.
    6. When you have enough information to answer the user's question,
       you MUST stop calling tools and instead output:

           Action: finish[answer="<a clear, friendly explanation in simple language.>"]

       The final answer should:
         - Speak to the user directly (“you”, “your situation”).
         - Start with a clear recommendation (for example: “You can probably keep your portfolio…”).
         - Then give 2–4 short reasons or factors to consider.
         - Avoid heavy jargon. If you must use a term, briefly explain it.
         - End with a gentle reminder that this is general information, not personal advice.

    Follow this exact step format, line by line:
    Thought: <your reasoning about what to do next>
    Action: <one tool call from above, or finish[...]>
""").strip()
